...
/**
 * Base implementation of an Assignment as an ArrayMap from RandomVariables to
 * Values.
 * 
...
public class Assignment extends ArrayMap<RandomVariable, Value> implements bn.core.Assignment {
	...
	/**
	 * Return true if this Assignment contains all the assignments in the given
	 * other Assignment. That is, the other Assignment is a subset of this one (or
	 * they are equal).
	 * <p>
	 * Maps are not Collections according to Java, so we need to implement this
	 * method ourselves. This is quadratic time but at least no memory when using
	 * ArrayMaps (which are ArraySets, themselves ArrayLists, of Map.Entrys).
	 * 
	 ...
	 * contains the same assignments without copying the RandomVariables or Values).
	 */
	...
	public static void main(String[] argv) {
		...
		System.out.format("%s containsAll %s? %s\n", assignment, assignment2, assignment.containsAll(assignment2));
		System.out.format("%s containsAll %s? %s\n", assignment2, assignment, assignment2.containsAll(assignment));
		System.out.format("%s equals %s? %s\n", assignment, assignment2, assignment.equals(assignment2));
