...
/**
 * Base implementation of a BayesianNetwork as a graph of Nodes each of which
 * corresponds to a RandomVariable with a CPT.
 * 
...
public class BayesianNetwork implements bn.core.BayesianNetwork {
	...
	/**
	 * The Set of Nodes representing this BayesianNetwork. We use an ArraySet since
	 * once we have created the network, it never changes.
	 */
	protected Set<Node> nodes = new ArraySet<Node>();

	/**
	 * Return the size (number of nodes == number of RandomVariables) in this
	 * BayesianNetwork.
	 */
	public int size() {
		return nodes.size();
	...
	/**
	 * Add a node for the given RandomVariable to this BayesianNetwork. Note: This
	 * is called when a {@code variable} entity is read in an XMLBIF file.
	 */
	...
	/**
	 * Return the RandomVariable with the given name from this BayesianNetwork. Only
	 * tests variables that implement the Named interface.
	 * <p>
	 * This is currently only used when reading a network from an XMLBIF file. If
	 * you used it more heavily, you'd probably want to index the variables more
	 * efficiently.
	 * 
	 * @throws NoSuchElementException
	 */
	...
	/**
	 * Return the RandomVariables in this BayesianNetwork as a List. Current
	 * implementation uses an ArrayList for this, and the order of the variables
	 * will be the same as the order they were added to the network.
	 */
	@Override
	...
	/**
	 * Connect the node for the given RandomVariable to the nodes for the given set
	 * of parent RandomVariables, with the given CPT. Note: This is called when a
	 * {@code definition} entity is read in an XMLBIF file.
	 */
	...
	/**
	 * Returns the Set of RandomVariables that are the children of the given
	 * RandomVariable. This is done in a really wasteful way, so you'd be better off
	 * doing something different for use in a certain kind of sampling...
	 */
	@Override
	...
	/**
	 * Returns the Set of RandomVariables that are the parents of the given
	 * RandomVariable. Also done inefficiently if you are calling it frequently.
	 */
	@Override
	...
	/**
	 * Return the probability stored in the CPT for the given RandomVariable, given
	 * the Values of its parents in the given Assignment.
	 */
	...
	/**
	 * Sort the given list of RandomVariables topologically with respect to this
	 * BayesianNetwork, and return the resulting list. I thought about just having a
	 * {@code topsort} method, into which one would pass a collection of
	 * RandomVariables (e.g., from {@link BayesianNetwork#getVariables}), but there
	 * was just too much converting between RandomVariables and Nodes for that. The
	 * only reason you ever topsort is to topsort the entire list of variables once
	 * at the start of inference, so why not just do that here.
	 * <p>
	 * This implementation uses the DFS algorithm described in Wikipedia and
