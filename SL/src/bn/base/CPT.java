...
/**
 * Base implementation of a conditional probability table (CPT) as a Map from
 * Assignments to Distributions.
 * <p>
 * Per AIMA, for finite, discrete distributions, we use a table with one row for
 * every combination of Values for the parents (an Assignment), and a column for
 * each Value of the CPT's variable (a Distribution).
 * <p>
...
public class CPT implements bn.core.CPT {
	...
	/**
	 * Return the ``row'' (Distribution) for the given Assignment in this CPT. We
	 * can't just use the assignment as the key to find the row, since it may
	 * contain entries for other variables besides just our parents. Using ArrayMaps
	 * for Assignments, the entrySet has to be created from the underlying
	 * ArrayList. The default implementation in AbstractSet is probably expensive,
	 * making this expensive. Sigh.
	 */
	...
	/**
	 * Set the probability of the given Value for this CPT's RandomVariable, given
	 * the values of its parents in the given Assignment, to the double p.
	 */
	@Override
	public void set(Value value, Assignment assignment, double p) {
		// System.out.format("CPT.set: %s %s=%s : %f\n", assignment, this.variable,
		// value, p);
	 ...
	 * given the values of its parents in the given Assignment. Note that the
	 * assignment may contain values for variables other than this CPT's variable
	 * (but it must contain at least values for the parents or an error will occur).
	 * Throws IllegalArgumentException if there is no entry for the given Assignment
	 * and Value in this CPT.
	 */
	@Override
