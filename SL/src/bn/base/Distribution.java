 ...
 * AIMA uses arrays indexed by the values of the variable. We use a Map whose
 * keys are the values. One could instead map the values to integer indexes (by
 * imposing an ordering on them), and then use an array of doubles to represent
 * the distribution. That might be nice since one can't put primitive doubles in
 * a Map...
 */
public class Distribution extends ArrayMap<Value, Double> implements bn.core.Distribution {
	 ...
	 * Note that we don't actually enforce that the values in the Distribution are
	 * only those for the RandomVariable. But anyway, this is handy.
	 */
